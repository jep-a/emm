...
function MINIGAME.PRNotification(time)
    if not NotificationService.stickies["Race_PR"].children[1] then
        NotificationService.PushMetaText(MINIGAME.GetTime(time), "Race_PR", 4)
    else
        NotificationService.stickies["Race_PR"].children[1]:SetText(MINIGAME.GetTime(time))
    end
end
